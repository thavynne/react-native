...
export default function ListScreen() {
  ...
  const carregarDados = async () => {
    try {
      ...
      // Busca todos os documentos da coleção 'usuarios' ordenados por data de criação
      const q = query(collection(db, 'usuarios'), orderBy('criadoEm', 'desc'));
      const querySnapshot = await getDocs(q);
      
      const usuariosCarregados: Usuario[] = [];
      ...
      Alert.alert(
        'Erro', 
        'Não foi possível carregar os dados. Verifique sua conexão e as configurações do Firebase.'
      );
    } finally {
      setCarregando(false);
    }
          ...
          onPress: async () => {
            try {
              ...
              const deletePromises = querySnapshot.docs.map((documento) =>
                deleteDoc(doc(db, 'usuarios', documento.id))
              );
...
const styles = StyleSheet.create({
  ...
  justifyContent: 'center',
  alignItems: 'center',
  backgroundColor: 'rgba(0,0,0,0.1)', // fundo levemente esmaecido
},
loadingBox: {
  padding: 20,
  backgroundColor: '#fff',
